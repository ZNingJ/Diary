    ...
    private void diaryPreSave(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            if (StringUtil.isNotEmpty(diaryId)) {
                con = dbUtil.getCon();
                Diary diary = diaryDao.diaryShow(con, diaryId);  //通过diaryId查找指定的日记信息
                request.setAttribute("diary", diary);   //将查找到的日记信息存入request
            }
            request.setAttribute("mainPage", "diary/diarySave.jsp"); //设置mainPage是diary/diarySave.jsp，这个jsp中有使用diary
            request.getRequestDispatcher("mainTemp.jsp").forward(request, response); //内部转发到mainTemp.jsp中个jsp中有包含diary/diarySave.jsp
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            try {
